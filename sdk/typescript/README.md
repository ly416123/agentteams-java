# AgentTeams TypeScript SDK

该 SDK 对应 `openapi/agentteams-public.yaml` 的 v1.0 公共 API 基线，当前提供
Project 和 Task 的核心读写方法。

```ts
import { AgentTeamsClient } from '@agentteams/typescript-sdk';

const client = new AgentTeamsClient({
  baseUrl: 'https://agentteams.example',
  accessToken: () => process.env.AGENTTEAMS_TOKEN,
});

const page = await client.listTasks({ pageSize: 20 });
```
