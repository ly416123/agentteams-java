...
/** Manager quota port backed by the Gateway quota gRPC service. */
public final class GrpcQuotaPort implements QuotaPort, AutoCloseable {
    ...
    public GrpcQuotaPort(ManagedChannel channel, String managerId) {
        this(channel, managerId, Clock.systemUTC(), Duration.ofSeconds(3), () -> "");
    }

    public GrpcQuotaPort(ManagedChannel channel, String managerId, Clock clock,
            Duration timeout, Supplier<String> traceparent) {
        this(channel, QuotaServiceGrpc.newBlockingStub(Objects.requireNonNull(channel, "channel")), managerId,
                clock, timeout, traceparent);
    }

    GrpcQuotaPort(ManagedChannel channel, QuotaServiceGrpc.QuotaServiceBlockingStub stub,
            String managerId, Clock clock, Duration timeout, Supplier<String> traceparent) {
        this.channel = Objects.requireNonNull(channel, "channel");
        this.stub = Objects.requireNonNull(stub, "stub");
        this.managerId = requireText(managerId, "managerId");
    ...
    public QuotaLease acquire(String tenantId, String projectId, long estimatedTokens) {
        ...
        AcquireQuotaResponse response = stub.withDeadlineAfter(timeout.toMillis(), TimeUnit.MILLISECONDS)
                .acquire(AcquireQuotaRequest.newBuilder().setMetadata(metadata()).setProtocolVersion(VERSION)
                        .setTenantId(tenantId).setProjectId(projectId).setIdempotencyKey(idempotencyKey)
                        .setEstimatedTokens(estimatedTokens).setMaxConcurrent(1).setDeadline(deadline()).build());
        ...
        if (!response.getAccepted()) throw new QuotaRejectedException(dimension(response.getRejectionDimension()));
        if (response.getReservationId().isBlank()) throw new IllegalStateException("quota reservation_id is required");
        return QuotaLease.idempotent(() -> release(tenantId, projectId, response.getReservationId()));
    }
    ...
    private void release(String tenantId, String projectId, String reservationId) {
        var response = stub.withDeadlineAfter(timeout.toMillis(), TimeUnit.MILLISECONDS)
                .release(ReleaseQuotaRequest.newBuilder().setMetadata(metadata()).setProtocolVersion(VERSION)
        ...
        if (response.getProtocolError() != QuotaProtocolError.QUOTA_PROTOCOL_ERROR_UNSPECIFIED
                || !response.getAccepted()) throw new IllegalStateException("quota release failed: " + response.getProtocolError());
    }

    private EventMetadata metadata() {
        return EventMetadata.newBuilder().setEventId(UUID.randomUUID().toString()).setAgentId(managerId)
                .setTraceparent(traceparent.get() == null ? "" : traceparent.get()).build();
    }
