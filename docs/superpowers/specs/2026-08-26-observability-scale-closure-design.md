...
**资源 ACK 进度（2026-08-28）：** `ConfigApplied` 已增加资源级结构化结果，Gateway/NATS/Control Plane 完成透传，Control Plane 以 V50 表持久化并按 binding/snapshot/config version 栅栏拒绝旧结果。Worker 已完成绑定字段校验、Skill 下载/digest 校验和配置应用流程报告；MCP 发现观测已增加按 server revision/实例隔离的 PostgreSQL 快照、过期聚合、健康探测写入和 Worker AgentScope runtime Port 注册。集中告警、预算预测和 L6 长压测仍未完成。
