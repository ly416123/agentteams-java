...
- [x] **步骤 3：持续推进产品阶段。** 基础切片和当前已纳入的产品阶段状态如下：
  ...
  4. [x] Team CRD 与调度：已完成 Fabric8 informer、稳定 Team 身份、PostgreSQL 幂等同步、成员替换/删除、runtime/capability/approval/concurrency policy 和独立只读 RBAC；确定性 Spring/PostgreSQL 验收已通过，Kind 冒烟脚本已提供。
...
当前未完成项主要是生产化能力和外部服务联调：OIDC/mTLS 证书与验证器、完整 RBAC/Secret rotation、Matrix/Tuwunel 实例、Prometheus Operator，以及完整灾备和故障注入验收。Team Kind 冒烟仍需要两个真实 READY Agent 作为运行前置条件；当前集群只有一个 READY Agent，因此脚本尚未伪造第二个 Agent 运行。
