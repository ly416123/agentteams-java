...
当前未完成项主要是生产化能力和外部服务联调：OIDC/mTLS 证书与验证器、完整 RBAC/Secret rotation、Matrix/Tuwunel 实例、Prometheus Operator，以及完整灾备和故障注入验收。Team Kind 冒烟已在两个真实 READY Agent 上完成；脚本仍要求显式传入两个 Agent UUID，避免将占位 Agent 当作真实 Worker。
