...
- [x] **步骤 3：持续推进产品阶段。** 基础切片和当前已纳入的产品阶段状态如下：
  ...
  5. [ ] OIDC/mTLS/RBAC/Secret rotation：已完成可配置 OIDC JWT 签名、issuer、audience、过期时间和租户/权限 claim 映射；已完成 Kind 本地 Gateway↔Worker 双向 mTLS、证书挂载验证及 Workload ServiceAccount 隔离；生产环境完整资源级 RBAC、证书签发/轮换仍待补齐。
