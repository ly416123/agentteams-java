...
- [x] **步骤 3：持续推进产品阶段。** 基础切片和当前已纳入的产品阶段状态如下：
  ...
  5. [ ] OIDC/mTLS/RBAC/Secret rotation：已完成可配置 OIDC JWT 签名、issuer、audience、过期时间和租户/权限 claim 映射；已完成 Kind 本地 Gateway↔Worker 双向 mTLS、证书挂载验证、Workload ServiceAccount 隔离，以及按安装命名空间收紧 Operator RBAC；已补齐稳定 Secret 名称、可选 Reloader 滚动刷新注解、生产 Secret 契约和 OIDC JWKS 轮换说明；多命名空间授权设计仍待补齐。
