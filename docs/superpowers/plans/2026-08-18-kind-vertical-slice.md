...
- [x] **步骤 3：持续推进产品阶段。** 基础切片和当前已纳入的产品阶段状态如下：
  ...
  6. [ ] Matrix AppService：已完成 Control Plane 适配器和幂等处理，并补齐本地 Tuwunel、文件式 AppService registration、shared-token 校验、真实用户/房间完整任务生命周期 smoke 和 Kind CI；`cancel/retry/pause/approve/reject` 已接入带 scope、权限、版本和幂等保护的任务生命周期。生产部署仍待补齐。
