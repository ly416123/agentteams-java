...
- [x] **步骤 1：编写失败测试。** 在 `UsageBudgetServiceTest` 增加三个行为测试：有效观测达到 1 小时后按 `actual / elapsed * period` 预测；有效观测不足 1 小时返回 `INSUFFICIENT_DATA` 且预测为空；成本没有任何可计价观测时返回 `UNPRICED` 且预测为空。测试使用固定 Clock、显式 scope 和内存 Usage provider，不依赖系统时间。
...
- [x] **步骤 3：实现最少领域代码。** 新增 `UsageBudgetPolicy`、`UsageBudgetEvaluation` 和 `UsageBudgetService`；服务只接受已解析的 `AuthorizationService.Scope`，窗口按 UTC `period` 对齐，成本使用 `BigDecimal`，有效观测小于策略 forecast window 时禁止生成预测，所有无价格输入保持 `null` 金额和 `UNPRICED` 状态。成本状态先由独立 `CostObservation` 值对象表达，避免把数据库中的零金额当成无价格。
...
- [x] **步骤 5：Commit。** 本纵切的领域、持久化和 API 代码与测试在最终交付提交中统一提交，避免在单工作区拆分出不可独立编译的中间提交。

```bash
git add control-plane/src/main/java/io/agentteams/controlplane/usage \
  control-plane/src/test/java/io/agentteams/controlplane/usage/UsageBudgetServiceTest.java
```

## 任务 2：预算 PostgreSQL 持久化

- [x] **步骤 1：编写失败 SQL 测试。** 在 `JdbcUsageBudgetRepositoryTest` 锁定策略插入字段、`UPDATE ... WHERE version = ?`、评估事件指纹和查询不得包含 prompt/response/token 等敏感正文；在迁移测试中断言 V52 持久化 `cost_status`、V53 创建三张预算表。
...
- [x] **步骤 3：实现最少持久化。** 创建 V52 为审计表增加 `cost_status` 并回填旧数据为 `ESTIMATED`（仅对非零成本）或 `UNPRICED`（零成本成功调用）；创建 V53：策略保存 tenant/project、币种、周期秒数、软/硬阈值、forecast window、状态、version 和时间；评估保存窗口、实际/预测金额、状态和评估时间；事件表使用稳定 fingerprint 唯一约束。Repository 的更新必须返回新版本，expectedVersion 不匹配抛出现有 `OptimisticLockFailure`，金额列使用 `NUMERIC`，不保存供应商正文。
...
```bash
git add control-plane/src/main/resources/db/migration/V52__model_call_audit_cost_status.sql \
  control-plane/src/main/resources/db/migration/V53__usage_budget_forecast.sql \
  control-plane/src/test/java/io/agentteams/controlplane/persistence/FoundationRepositoryIT.java
```

## 任务 3：项目作用域 API 和交付

- [x] **步骤 1：编写失败 API 测试。** 在 `UsageBudgetControllerTest` 增加 `PUT /api/v1/usage/budgets/{policyId}`、`GET /api/v1/usage/budgets` 和 `GET /api/v1/usage/budgets/{policyId}/evaluations?limit=20` 测试；断言请求不能覆盖 tenant/project，expectedVersion 会透传，响应包含状态/金额/窗口而不包含审计正文，limit 仅允许 1 到 100。
...
- [x] **步骤 3：实现最少 API 和装配。** 新增 `UsageBudgetController` 与 `UsageBudgetService` 的 Spring Bean 装配；`PUT` 只从 PrincipalContext 取得 tenant/project，`GET` 只返回当前项目策略，评估查询限制 page size 并按窗口倒序返回。为 `/api/v1/usage` 复用现有 `USAGE_READ`/`QUOTA_WRITE` 授权，不新增绕过 scope 的内部接口。
...
- [x] **步骤 5：本机 Docker/Colima 验收。** 使用 `source deploy/dev-env.sh` 加载 Colima Docker socket，FoundationRepositoryIT 已验证迁移和预算表写入；本任务不注入外部计费凭据，也不伪造外部服务。

- [x] **步骤 6：同步文档并提交。** 文档只将策略、线性预测、`UNPRICED`、`INSUFFICIENT_DATA` 和项目作用域查询标记为完成；完整维度覆盖率、预算通知投递、最终账单和 L6 生产压力仍保持未完成。

```bash
git add docs/superpowers/plans/2026-08-28-usage-budget-forecast-plan.md \
  ...
  control-plane/src/main/java/io/agentteams/controlplane/audit/JdbcModelCallAuditRecorder.java \
  control-plane/src/main/java/io/agentteams/controlplane/usage/UsageQueryService.java \
  control-plane/src/test/java/io/agentteams/controlplane/audit/JdbcModelCallAuditRecorderTest.java \
  manager/src/main/java/io/agentteams/manager/JdbcModelCallAuditor.java \
  manager/src/test/java/io/agentteams/manager/JdbcModelCallAuditorTest.java
