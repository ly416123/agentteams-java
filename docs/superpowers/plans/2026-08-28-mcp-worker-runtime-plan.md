...
- [ ] **步骤 1：同步规格状态**：更新 observability 和 remaining-capabilities 路线图，明确已完成 Worker runtime Port，保留集中告警、外部凭证真实注入和 L6 长期运行边界。
- [ ] **步骤 2：运行模块验证**：`source deploy/dev-env.sh && mvn -q test`、`python3 -m unittest discover -s scripts -p 'test_*.py'`、`python3 -m py_compile scripts/*.py`、`bash scripts/validate-backup-scripts.sh`、`python3 scripts/validate-production-values.py`、`python3 scripts/validate-production-network.py`、`python3 scripts/validate-architecture-map.py`、`helm lint deploy/helm/agentteams-java`、`git diff --check`。
- [ ] **步骤 3：运行本地 Docker 验证**：`source deploy/dev-env.sh && mvn -q -Pintegration-tests verify`，确认 Flyway/Control Plane/Agent Worker 集成启动成功。
- [ ] **步骤 4：检查敏感信息与范围**：确认 diff 不含 token、Authorization、完整 MCP 响应、endpoint 标签或新分支/worktree。
