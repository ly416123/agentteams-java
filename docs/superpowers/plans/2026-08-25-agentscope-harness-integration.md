...
> **面向 AI 代理的工作者：** 必需子技能：使用 \`subagent-driven-development\`（推荐）或 \`executing-plans\` 逐任务实现此计划。步骤使用复选框跟踪进度；截至 2026-08-25，任务 1-8 已完成，真实 DeepSeek 冒烟因本机未设置环境变量而按设计跳过。
...
- [x] **步骤 4：固定 AgentScope Maven 版本并检查依赖树。**
...
- [x] **步骤 2：实现 AgentScope Model Adapter。**
...
- [x] **步骤 4：运行 Manager 和 Control Plane 定向测试。**
...
- [x] **步骤 4：本地执行可选真实模型冒烟（无本地 Key，按设计跳过）。**
