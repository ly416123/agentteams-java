...
**目标：** 在当前 `8b070c8` 基线上完成 Kubernetes Sandbox、AgentScope Worker 路由、Manager 正式服务、Team Revision/Effective Config，以及对应 Helm 安全配置。
...
- [ ] **步骤 4：实现 AgentScope 配置工厂和只读 Sandbox 边界**
...
预期：退出码为 0；无 Docker 时仅跳过明确标记的 Testcontainers 测试，并记录该环境限制。

- [ ] **步骤 2：运行迁移、Helm 和静态检查**

运行：`mvn -q -Pintegration-tests verify`、`helm lint deploy/helm/agentteams-java`、`helm template agentteams deploy/helm/agentteams-java --namespace agentteams`、`git diff --check`。

预期：可运行的本地检查全部通过；需要 Docker/Kind 的检查明确区分 skipped 与 passed。

- [ ] **步骤 3：运行 Kind 验收（环境可用时）**
