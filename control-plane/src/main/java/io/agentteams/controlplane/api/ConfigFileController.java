...
public final class ConfigFileController {
    ...
    public ConfigFileController(ConfigUploadService uploads, ConfigSnapshotRepository snapshots) {
    ...
    public PrepareResponse prepare(@PathVariable UUID snapshotId, @RequestBody PrepareRequest request) {
        requireSnapshotScope(snapshotId);
    ...
    private void requireSnapshotScope(UUID snapshotId) {
        if (PrincipalContext.current().isPresent()) {
            ConfigSnapshot snapshot = snapshots.findById(snapshotId)
