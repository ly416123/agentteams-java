...
class TaskSandboxStatusMapperTest {
    ...
    void mapsAJobFailureToStableFailedStatusWithoutContainerDetails() {
        Job failed = new JobBuilder().withStatus(new JobStatusBuilder().withFailed(1).withConditions(
    ...
    void mapsAFailedConditionEvenBeforeTheJobFailureCountIsUpdated() {
        Job failed = new JobBuilder().withStatus(new JobStatusBuilder().withConditions(
    ...
    void requiresEndpointAndRunnerHealthBeforeReady() {
        Job running = new JobBuilder().withStatus(new JobStatusBuilder().withActive(1).build()).build();
