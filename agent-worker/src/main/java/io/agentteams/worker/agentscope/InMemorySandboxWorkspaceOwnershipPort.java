...
public final class InMemorySandboxWorkspaceOwnershipPort implements SandboxWorkspaceOwnershipPort {
    ...
    @Override
    public synchronized void releaseSandbox(String providerSandboxId, WorkspaceOwner owner) {
        if (owner.equals(sandboxOwners.get(providerSandboxId))) {
            sandboxOwners.remove(providerSandboxId);
        }
    }

    @Override
    public synchronized void releaseWorkspace(Path workspacePath, WorkspaceOwner owner) {
        Path normalized = normalize(workspacePath);
        if (owner.equals(workspaceOwners.get(normalized))) {
            workspaceOwners.remove(normalized);
        }
    }
